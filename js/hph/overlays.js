	...
	if(resultat_json.resultats.length != 0){
		...
		for(i=0; i < resultat_json.resultats.length; i++){
			...
			for(var j=0; j < resultat_json.resultats[i].geo.latitudes.length; j++){
				...
				if(resultat_json.resultats[i].geo.heures[j]<=fake_current_time){
			...
			if(latitudes.length>0){
				var index_last = latitudes.length-1;
				if(photos[index_last]!="none"){
					type="photo";
				}
				else{
					type="position";
				}
				add_last(type,latitudes[index_last],longitudes[index_last],id_equipe,nom_equipe,photo_equipe,distance,heures[index_last],photos[index_last],messages[index_last]);
			}
			//on relit toutes les positions entre elles
			if(latitudes.length>0){
				//on affiche le parcours de l'équipe
...
function get_teams_sorted(){
	...
	if(resultat_json.resultats.length != 0){
		...
		var latitudes = new Array;
		var longitudes = new Array;
		var id_equipe,nom_equipe,distance;
		
		var table_teams = new Array(resultat_json.resultats.length);
		...
		for(i=0; i < resultat_json.resultats.length; i++){	
			...
			table_teams[i] = new Array(3);
			table_teams[i][0] = id_equipe;
			table_teams[i][1] = nom_equipe;
			table_teams[i][2] = distance;
		...
		//on trie le tableau
		table_teams.sort( function by_distance(a,b) {
			if (a[2] > b[2] ) { return -1; }
			if (a[2] < b[2] ) { return 1; }
			return 0;
		});
		...
		//on l'affiche
		for(var k=0;k<table_teams.length;k++){
			id_equipe=table_teams[k][0];
			nom_equipe=table_teams[k][1];
			distance=table_teams[k][2];
...
function add_last(type,latitude,longitude,id_equipe,nom_equipe,photo_equipe,distance,heure,photo,message){
	...
	switch (type) {
		...
		case "photo":
			...
			var contenu_info = "<div style='min-height:"+infowindow_height+"px;min-width:"+infowindow_height+"px'><center><b>"+message+"</b><br/>"+il_y_a+"<br/><a href='./galerie.php'><img src='../images/th."+photo+"'></a></center></div>";
